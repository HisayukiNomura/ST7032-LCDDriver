...
/// @brief ST7032のコマンド定義。ディスプレイの消去。 
/// @details ライブラリを使うだけの場合はこのコマンドを直接使用せず、lcd_ClearDisplay()を使用する。
const static int LCD_CLEARDISPLAY = 0x01;

/// @brief ST7032のコマンド定義。カーソルを(0,0)に移動。
/// @details ライブラリを使うだけの場合はこのコマンドを直接使用せず、lcd_ReturnHome()を使用する。
const static int LCD_RETURNHOME = 0x02; 

/// @brief ST7032のコマンド定義。文字の表示方向指定
/// @details ライブラリを使うだけの場合はこのコマンドを直接使用せず、lcd_EntryModeSet(bool isDisplayToLeft)を使用する。
const static int LCD_ENTRYMODESET = 0x04;

/// @brief ST7032のコマンド定義。ディスプレイやカーソルの表示制御
/// @details ライブラリを使うだけの場合はこのコマンドを直接使用せず、int lcd_CursorDisplay(bool isDisp)を使用する。
const static int LCD_DISPLAYONOFF = 0x08;              

/// @brief ST7032のコマンド定義。ディスプレイの機能設定  
/// @details ライブラリを使うだけの場合はこのコマンドを直接使用せず、int lcd_FunctionSet(bool is8Bit , bool is2Line ,bool isExtInstruction);を使用する。\n
/// Instruction Tableの変更（isExtInstruction）だけを行いたい場合、int lcd_NormalMode()とint lcd_ExtendMode()を使用することもできる。
const static int LCD_FUNCTIONSET = 0x20;               

/// @brief ST7032のコマンド定義。操作するディスプレイメモリ（文字やアイコン）のアドレス設定
/// @details ライブラリを使うだけの場合はこのコマンドを直接使用せず、アイコンの設定関数や、文字列の表示関数を使用する。
const static int LCD_SETDDRAMADDR = 0x80;              

// INSTRUCTION_TABLE が０のときのコマンド
/// @brief ST7032のコマンド(INSTRUCTION_TABLEが0の場合)の定義。カーソルの移動方向指定
/// @details ライブラリを使うだけの場合はこのコマンドを直接使用せず、int lcd_DisplayShift(int8_t ShiftCnt)関数を使用する。
const static int LCD_TBL0_CURDISPSHIFT = 0x10;

/// @brief ST7032のコマンド(INSTRUCTION_TABLEが0の場合)の定義。表示文字の設定
/// @details このライブラリではまだ使用していない。
const static int LCD_TBL0_SETCGRAM = 0x40;

// INSTRUCTION_TABLE が１のときのコマンド(拡張コマンド)
/// @brief ST7032のコマンド(INSTRUCTION_TABLEが1の場合)の定義。内部オシレータ設定
/// @details このコマンドは直接使用せず、int lcd_InternalOSCSet(bool  isBs, uint8_t OSCFreq)関数を使用する
const static int LCD_TBL1_INTOSC = 0x10; 

/// @brief ST7032のコマンド(INSTRUCTION_TABLEが1の場合)の定義。表示するアイコンのアドレス設定              
/// @details このコマンドは直接使用せず、int lcd_IconSet(bool isDisp ,LCD_ICON icon)関数を使用する
const static int LCD_TBL1_SETICON = 0x40;

/// @brief ST7032のコマンド(INSTRUCTION_TABLEが1の場合)の定義。アイコン表示や電源ブースト設定など
/// @details このコマンドは直接使用せず、int lcd_ContrastSet(int contrast , bool is_PowerIconCtrl_IconOn , bool is_PowerIconCtrl_Boost)や、lcd_Sleep関数を使用する。
const static int LCD_TBL1_POWERICONCTRL = 0x50;

/// @brief ST7032のコマンド(INSTRUCTION_TABLEが1の場合)の定義。ボルテージフォロア回路の設定
/// @details このコマンドは直接使用せず、lcd_FollowerControlSet(bool isOnOff,uint8_t ampRatio)を使用すべきだが、そもそもこの値を変更する理由が無いので無視してもよい。
const static int LCD_TBL1_FOLLOWERCTRL = 0x60;

/// @brief ST7032のコマンド(INSTRUCTION_TABLEが1の場合)の定義。液晶コントラストの設定
/// @details このコマンドは直接使用せず、lint lcd_ContrastSet(int contrast , bool is_PowerIconCtrl_IconOn , bool is_PowerIconCtrl_Boost)使用すべきだが、そもそもこの値を変更する理由が無いので無視してもよい。
const static int LCD_TBL1_FOLLOWERCONTRAST = 0x70;
...
const static int LCD_ENTRYMODESET_SHIFTINCREMENT = 0x01;

/// @brief LCD_ENTRYMODESETコマンドの時使用するオプション。
/// @details このフラグがオンでDDRAM書き込みを行った場合、I/D値（I/D="1"：左シフト、I/D="0"：右シフト）により表示全体のシフトとなる。
//  このオプションは使用せず、lcd_EntryModeSet(bool isDisplayToLeft)を使用する。
const static int LCD_ENTRYMODESET_LEFT = 0x02;             // 

// LCD_DISPLAYONOFFのときのフラグ
/// @brief LCD_DISPLAYONOFFコマンド時のオプション。カーソル位置の文字が点滅する。（■と文字が交互に表示される）
/// @details ライブラリを使うだけの場合はこのコマンドを直接使用せず、カーソル表示の制御はlcd_CursorDisplay(bool isDisp)を使用する。液晶のオフはlcd_Sleepなどを使用する。
const static int LCD_DISPLAYONOFF_CURSORPOS_ON= 0x01;

/// @brief LCD_DISPLAYONOFFコマンド時のオプション。カーソル位置に下線が表示される。
/// @details ライブラリを使うだけの場合はこのコマンドを直接使用せず、カーソル表示の制御はlcd_CursorDisplay(bool isDisp)を使用する。
const static int LCD_DISPLAYONOFF_CURSOR_ON = 0x02;

/// @brief LCD_DISPLAYONOFFコマンド時のオプション。ディスプレイ全体が表示される。
/// @details ライブラリを使うだけの場合はこのコマンドを直接使用せず、カ液晶のオフはlcd_Sleepなどを使用する。
const static int LCD_DISPLAYONOFF_DISPLAY_ON = 0x04;


// LCD_FUNCTIONSETのときのフラグ

/// @brief LCD_FUNCTIONSETのときのオプション。８ビットモードで動作させる。このライブラリは４ビットモードをサポートしていないので、常に指定する。
/// @dertails ライブラリを使うだけの場合はこのコマンドを直接使用せず、int lcd_FunctionSet(bool is8Bit , bool is2Line ,bool isExtInstruction);を使用する。
const static int LCD_FUNC_8BITMODE = 0x10;

/// @brief LCD_FUNCTIONSETのときのオプション。２行モードを指定する。これを指定せず、LCD_FUNC_DOUBLEHEIGHTを指定すると、２倍の高さの文字が１行で表示される。
/// @dertails ライブラリを使うだけの場合はこのコマンドを直接使用せず、int lcd_FunctionSet(bool is8Bit , bool is2Line ,bool isExtInstruction);を使用する。
const static int LCD_FUNC_2LINE = 0x08;

/// @brief LCD_FUNCTIONSETのときのオプション。２行モードを指定する。これを指定せず、LCD_FUNC_DOUBLEHEIGHTを指定すると、２倍の高さの文字が１行で表示される。
/// @dertails ライブラリを使うだけの場合はこのコマンドを直接使用せず、int lcd_FunctionSet(bool is8Bit , bool is2Line ,bool isExtInstruction);を使用する。
const static int LCD_FUNC_DOUBLEHEIGHT = 0x04;

/// @brief LCD_FUNCTIONSETのときのオプション。１のときはExtention instruction tableを使用する。
/// @dertails このコマンドをライブラリを使うだけの場合はこのコマンドを直接使用せず、int lcd_FunctionSet(bool is8Bit , bool is2Line ,bool isExtInstruction);を使用するか、
/// int lcd_NormalMode()　や　int lcd_ExtendMode()を使って命令モードを変更する。
const static int LCD_FUNC_INSTTBL_SELECT = 0x01;       // １のときはExtention instruction tableを使用する。

/// @brief LCD_SETDDRAMADDR のときのフラグ。
/// @details ライブラリを使うだけの場合はこのコマンドを直接使用せず、アイコンの設定関数や、文字列の表示関数を使用する。
...
// のシフトを繰り返す場合は、各行毎にシフトします。 表示シフトを行っても、アドレスカウン
// タの内容は変更されません。
/// @brief LCD_TBL0_CURDISPSHIFTコマンドのときのオプション。カーソルを左に移動させる。
const static int LCD_TBL0_CURSOR_LEFT  = 0b00000000;

/// @brief LCD_TBL0_CURDISPSHIFTコマンドのときのオプション。カーソルを右に移動させる。
const static int LCD_TBL0_CURSOR_RIGHT = 0b00000100;

/// @brief LCD_TBL0_CURDISPSHIFTコマンドのときのオプション。画面全体を左に移動させる。
const static int LCD_TBL0_DISPLAY_LEFT = 0b00001000;

/// @brief LCD_TBL0_CURDISPSHIFTコマンドのときのオプション。画面全体を右に移動させる。
const static int LCD_TBL0_DISPLAY_RIGHT= 0b00001100;


// LCD_TBL0_SETCGRAM
/// @brief CD_TBL0_SETCGRAMの際に、アイコンをオンにするビットを立てる際のマスク。
/// @details 現在使用していない
const static int LCD_TBL0_SETCGRAM_MASK = 0b00111111;
...
const static int LCD_TBL1_INTOSC_BS_1BY4 = 0x08;

/// @brief LCD_TBL1_INTOSCの際のオプション。このマスク（下位３ビット）内蔵オシレータの調整値を指定する。
/// @details ライブラリを使うだけの場合は直接使用せず、lcd_InternalOSCSet(bool  isBs, uint8_t OSCFreq)を使用する。
const static int LCD_TBL1_INTOSC_FREQMASK = 0b00000111;


/// @brief LCD_TBL1_SETICON の際の、アイコンビット指定。アドレスは0～16、ビットは各5ビット、16x5=80種類のアイコンが制御できる。この、ビット指定のビットマスク。
/// @details ライブラリを使うだけの場合は直接使用せず、int lcd_IconSet(bool isDisp ,LCD_ICON icon)を使用する。
const static int LCD_TBL1_SETICON_MASK = 0b00011111;

/// @brief  LCD_TBL1_POWERICONCTRLを使用する際のオプション。アイコン表示を行うかどうかの指定。
/// @details ライブラリを使うだけの場合は直接使用せず、lcd_ContrastSet(int contrast , bool is_PowerIconCtrl_IconOn , bool is_PowerIconCtrl_Boost)を使用し、第２引数に指定する。
const static int LCD_TBL1_POWERICONCTRL_ON = 0x08;

/// @brief  LCD_TBL1_POWERICONCTRLを使用する際のオプション。電源ブーストを行うかどうかを指定する。
...
const static int LCD_TBL1_POWERICONCTRL_BOOST = 0x04;

/// @brief LCD_TBL1_POWERICONCTRLを使用する際のオプション。液晶コントラスト指定の上位２ビット用のビットマスク。
/// @details ライブラリを使うだけの場合は直接使用せず、lcd_ContrastSet(int contrast , bool is_PowerIconCtrl_IconOn , bool is_PowerIconCtrl_Boost)を使用し、第１引数に指定する。
const static int LCD_TBL1_POWERICONCTRL_CONTRAST_C5C4_MASK = 0b00000011;



/// @brief LCD_TBL1_FOLLOWERCTRLを使用する際のオプション。電圧フォロア回路をオンにする。通常はオン指定。
/// @details ライブラリを使うだけの場合は直接使用せず、int lcd_FollowerControlSet(bool isOnOff,uint8_t ampRatio)を使用し、第１引数に指定する。
const static int LCD_TBL1_FOLLOWERCTRL_ON = 0x08;

/// @brief LCD_TBL1_FOLLOWERCTRLを使用する際のオプション。増幅調整値を指定するためのマスク。
/// @details ライブラリを使うだけの場合は直接使用せず、int lcd_FollowerControlSet(bool isOnOff,uint8_t ampRatio)を使用し、第２引数に指定する。
const static int LCD_TBL1_FOLLOWERCTRL_AMPRATIO_MASK = 0b00000111;


// LCD_TBL1_FOLLOWERCONTRAST　を使用する際のオプション。液晶のコントラストの下位4ビットを指定するためのマスク。（LCD_TBL1_POWERICONCTRLで、上位2ビットを指定）
/// @details ライブラリを使うだけの場合は直接使用せず、lcd_ContrastSet(int contrast , bool is_PowerIconCtrl_IconOn , bool is_PowerIconCtrl_Boost)を使用し、第３引数に指定する。
const static int LCD_TBL1_FOLLOWERCONTRAST_MASK = 0b00001111;
...
{
    int iRet = lcd_send_byte(LCD_RETURNHOME);
...
{
    uint8_t val = LCD_ENTRYMODESET;
    if (a_isDisplayToLeft) {
        val |= (LCD_ENTRYMODESET_LEFT | LCD_ENTRYMODESET_SHIFTINCREMENT);
    } else {
        //val |= LCD_ENTRYMODESET_SHIFTINCREMENT;
    }
...
{
    int iRet;
    lcdSetting.isFunc_ISMode = (mode & LCD_FUNC_INSTTBL_SELECT) != 0;
    lcdSetting.isFunc_2LINE =  (mode & LCD_FUNC_2LINE) != 0;
    lcdSetting.isFunc_DoubleHeight = (mode & LCD_FUNC_DOUBLEHEIGHT) != 0;
    lcdSetting.isFunc_8Bit =  (mode & LCD_FUNC_8BITMODE) != 0;
    iRet = lcd_send_byte(LCD_FUNCTIONSET | mode);
    return iRet;
}
/**
...
{
    uint8_t opt = 0;
    opt |= is8Bit ? LCD_FUNC_8BITMODE : 0;
    opt |= is2Line ? LCD_FUNC_2LINE  : LCD_FUNC_DOUBLEHEIGHT;
    opt |= isExtInstruction ? LCD_FUNC_INSTTBL_SELECT : 0;
...
int lcd_NormalMode()
{
    if (lcdSetting.isFunc_ISMode) { // 現在、拡張モードにある場合
        uint8_t val = 0;
        val |= lcdSetting.isFunc_2LINE ? LCD_FUNC_2LINE:0;
        val |= lcdSetting.isFunc_DoubleHeight ? LCD_FUNC_DOUBLEHEIGHT:0;
        val |= lcdSetting.isFunc_8Bit ? LCD_FUNC_8BITMODE:0;
        int iRet;
...
int lcd_InternalOSCSet(bool  isBs, uint8_t OSCFreq)
{
    int iSendBytes;
    int iRet;
    uint8_t val = LCD_TBL1_INTOSC | (LCD_TBL1_INTOSC_FREQMASK & 0x04);

    iRet = lcd_ExtendMode();
    iSendBytes += iRet;
    if (isBs) {
        val |= LCD_TBL1_INTOSC_BS_1BY4;
    }
...
int lcd_CursorMode(bool isDisplayOn , bool isUnderLine , bool isBlink)
{
    ...
    mode |= isDisplayOn ? LCD_DISPLAYONOFF_DISPLAY_ON : 0;
    mode |= isUnderLine ? LCD_DISPLAYONOFF_CURSOR_ON : 0;
    mode |= isBlink ? LCD_DISPLAYONOFF_CURSORPOS_ON : 0;

    iRet = lcd_send_byte(LCD_DISPLAYONOFF | mode);
...
int lcd_CursorDisplay(bool isDisp)
{
    ...
    if (isDisp) {       // カーソル表示を行う場合、事前に保存してあったカーソルモードを送信する
        ...
        iRet = lcd_send_byte(LCD_DISPLAYONOFF | (lcdSetting.isDisplayOn ? LCD_DISPLAYONOFF_DISPLAY_ON:0));
        lcdSetting.isCursorDisplay = false;
    }
    return iRet;
}
...
int lcd_FollowerControlSet(bool isOnOff,uint8_t ampRatio)
{
    int iSendBytes;
    int iRet;
    uint8_t val = LCD_TBL1_FOLLOWERCTRL | (LCD_TBL1_FOLLOWERCTRL_AMPRATIO_MASK & ampRatio) ;

    iRet = lcd_ExtendMode();
    iSendBytes += iRet;
    if (isOnOff) {
        val |=  LCD_TBL1_FOLLOWERCTRL_ON ;
    };
...
int lcd_ContrastPowerIconSet(int contrast , bool is_PowerIconCtrl_IconOn , bool is_PowerIconCtrl_Boost)
{
    ...
    volatile int iRet;
    uint8_t otherBit = 0;
    otherBit |= is_PowerIconCtrl_IconOn ? LCD_TBL1_POWERICONCTRL_ON : 0;
    otherBit |= is_PowerIconCtrl_Boost ? LCD_TBL1_POWERICONCTRL_BOOST : 0;
    ...
    iRet = lcd_send_byte(LCD_TBL1_FOLLOWERCONTRAST | (LCD_TBL1_FOLLOWERCONTRAST_MASK & contrast) );
    if (iRet != 2) return iRet;
    // 次に、LCD_TBL1_POWERICONCTRLで、上位2ビットを送信
    iRet = lcd_send_byte(LCD_TBL1_POWERICONCTRL | otherBit | (LCD_TBL1_POWERICONCTRL_CONTRAST_C5C4_MASK & (contrast >> 4)) );
    if (iRet != 2) return iRet;
    lcd_NormalMode();
    return 4;
}
...
int lcd_IconSetRAW(bool isDisp , uint8_t iconAddr , uint8_t bits)
{
    ...
    iRet = lcd_ExtendMode();
    iSendBytes += iRet;
    iRet = lcd_send_byte(LCD_TBL1_SETICON | iconAddr);
    iSendBytes += iRet;
...
int lcd_IconSetAll(bool isDisplay)
{
    ...
    if (isDisplay) {
        ...
        for (int i = 0;i<16;i++) {
            ...
            iRet = lcd_send_byte(LCD_TBL1_SETICON | i);
            iSendBytes += iRet;
...
int lcd_DisplayShift(int8_t ShiftCnt)
{
    ...
    uint8_t moveOpt = 0;
    if (ShiftCnt <0) {
        moveOpt |= (LCD_TBL0_DISPLAY_LEFT |  LCD_TBL0_CURSOR_LEFT );
    } else {
        moveOpt |= (LCD_TBL0_DISPLAY_RIGHT | LCD_TBL0_CURSOR_RIGHT);
    }
    for (int8_t i=0;i<movecnt;i++) {
        iRet = lcd_send_byte(LCD_TBL0_CURDISPSHIFT | moveOpt);
        iSendBytes += iRet;
    }
    return iSendBytes;
}
...
int lcd_MoveCursor(int8_t MoveCnt)
{
    ...
    uint8_t moveOpt = 0;
    if (MoveCnt <0) {
        moveOpt |= LCD_TBL0_CURSOR_LEFT;
    } else {
        moveOpt |= LCD_TBL0_CURSOR_RIGHT;
    }
    for (int8_t i=0;i<movecnt;i++) {
        iRet = lcd_send_byte(LCD_TBL0_CURDISPSHIFT | moveOpt);
        iSendBytes += iRet;
...
int lcd_Sleep(bool isSleep)
{
    ...
    if (isSleep) {
        ...
        iSendBytes += iRet;
        iRet = lcd_send_byte(LCD_TBL1_FOLLOWERCTRL);        // ボルテージフォロア回路オフ、増幅率はゼロ
        iSendBytes += iRet;
        iRet = lcd_send_byte(LCD_TBL1_POWERICONCTRL);       // アイコン表示オフ、電圧増幅回路オフ、コントラスト上位２ビットをゼロ
        iSendBytes += iRet;
...
void lcd_init() 
{
    ...
    iRet = lcd_FunctionSet(LCD_FUNC_8BITMODE | LCD_FUNC_2LINE| LCD_FUNC_INSTTBL_SELECT);
